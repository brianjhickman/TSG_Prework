function clearErrors() {
    for (var loopCounter = 0;
        ...
        loopCounter++) {
        // Test to see if form error:  IF (TRUE) condition when there IS a form error
        // So if statement executes code when particular form element DOES have an error            
        if (document.forms["numberFun"].elements[loopCounter].parentElement.className.indexOf("has-") != -1) {
            // Removes the "has-error" CSS class that's been added because of the error
            // And resets the CSS class to simply "form-group"
